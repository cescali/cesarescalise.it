...
export async function getPost(slug: string): Promise<WPPost | null> {
  const res = await fetch(`${WP_API}/posts?slug=${slug}&_embed=wp:featuredmedia&status=publish`);
...
export async function getTotalPostPages(perPage = 10): Promise<number> {
  const res = await fetch(`${WP_API}/posts?per_page=${perPage}&status=publish`);
  if (!res.ok) return 1;
...
export async function getGalleries(): Promise<Gallery[]> {
  const res = await fetch(`${CUSTOM_API}/galleries`);
...
export async function getGallery(id: number): Promise<Gallery | null> {
  const res = await fetch(`${CUSTOM_API}/galleries/${id}`);
  if (!res.ok) return null;
  return res.json();
}

export async function getGalleryPics(galleryId: number): Promise<GalleryPic[]> {
  const res = await fetch(`${CUSTOM_API}/galleries/${galleryId}/pics`);
  if (!res.ok) return [];
  return res.json();
}
